...
def test_random_jaccard_multiple_samples_average():
    features = [f"gene_{idx}" for idx in range(25)]
    real_fold_change = np.linspace(0.5, 2.5, len(features)).tolist()
    real_fdr = [0.01 if idx % 2 == 0 else 0.2 for idx in range(len(features))]
    ...
    pred_fdr = [0.01 if idx % 3 == 0 else 0.2 for idx in range(len(features))]
    pred = pl.DataFrame(
        {
            "target": ["pert"] * len(features),
            "feature": features,
    ...
    pred_subset = (
        comparison.pred.data.filter(pl.col(target_col) == "pert")
        .filter(pl.col(feature_col).is_in(real_subset[feature_col]))
        .select([feature_col, comparison.pred.fdr_col])
    )
    pred_map = {row[0]: float(row[1]) for row in pred_subset.iter_rows()}
    ...
    for _ in range(num_samples):
        ...
        sampled_genes = [record.gene for record in sampled]
        real_de = {
            record.gene for record in sampled if record.fdr <= 0.05
        }
        pred_de = {
            gene for gene in sampled_genes if pred_map.get(gene, float("inf")) <= 0.05
        }
        union = real_de | pred_de
        expected_score = 0.0 if not union else len(real_de & pred_de) / len(union)
        total_expected += expected_score

    expected_average = total_expected / num_samples
    assert pytest.approx(expected_average) == score
